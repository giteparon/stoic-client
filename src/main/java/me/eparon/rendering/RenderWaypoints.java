...
import me.eparon.movement.pathFinding;
import me.eparon.StoicClient;
import java.util.ArrayList;

public class RenderWaypoints implements ModInitializer {
    private static RenderWaypoints instance;
    ...
    private static final RenderPipeline FILLED_THROUGH_WALLS = RenderPipelines.register(RenderPipeline.builder(RenderPipelines.DEBUG_FILLED_SNIPPET)
            .withLocation(Identifier.fromNamespaceAndPath(StoicClient.MOD_ID, "pipeline/debug_filled_box_through_walls"))
    ...
    // :::custom-pipelines:drawing-phase
    public static RenderWaypoints getInstance() {
        return instance;
    }
    ...
    private void renderWaypoint(WorldRenderContext context) {
        ...
        if (buffer == null) {
            ...
            System.out.println("buffer was empty, not anymore");
        }
        ArrayList<ArrayList<Float>> waypoints = pathFinding.waypointsArray;
        for(int i = 0; i < waypoints.size(); i++){
            if(waypoints.get(i).get(0) != null){
                renderFilledBox(matrices.last().pose(), buffer, waypoints.get(i).get(0), waypoints.get(i).get(1), waypoints.get(i).get(2), waypoints.get(i).get(0)  +1, waypoints.get(i).get(1) + 1, waypoints.get(i).get(2) + 1, 0f, 1f, 0f, 0.5f);
            }
    ...
    private GpuBuffer upload(MeshData.DrawState drawParameters, VertexFormat format, MeshData builtBuffer) {
        ...
        if (vertexBuffer == null || vertexBuffer.size() < vertexBufferSize) {
            ...
            vertexBuffer = new MappableRingBuffer(() -> StoicClient.MOD_ID + " example render pipeline", GpuBuffer.USAGE_VERTEX | GpuBuffer.USAGE_MAP_WRITE, vertexBufferSize);
        }
    ...
    private static void draw(Minecraft client, RenderPipeline pipeline, MeshData builtBuffer, MeshData.DrawState drawParameters, GpuBuffer vertices, VertexFormat format) {
        ...
        try (RenderPass renderPass = RenderSystem.getDevice()
                ...
                .createRenderPass(() -> StoicClient.MOD_ID + " example render pipeline rendering", client.getMainRenderTarget().getColorTextureView(), OptionalInt.empty(), client.getMainRenderTarget().getDepthTextureView(), OptionalDouble.empty())) {
